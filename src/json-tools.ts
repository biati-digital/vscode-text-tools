...
class JsonTools extends VSCodeTextTools {
	...
	init(): void {
		...
		disposable = vscode.commands.registerCommand(
			"biati.texttools.jsonstringencode",
			async () => {
		...
		disposable = vscode.commands.registerCommand(
			"biati.texttools.jsonstringdecode",
			async () => {
				const editor = vscode.window.activeTextEditor;
		...
		disposable = vscode.commands.registerCommand(
			"biati.texttools.jsonstringserialize",
			async () => {
				const editor = vscode.window.activeTextEditor;
		...
		disposable = vscode.commands.registerCommand(
			"biati.texttools.jsonstringunserialize",
			async () => {
				const editor = vscode.window.activeTextEditor;
	...
	jsonSerialize(text: string) {
		...
		try {
			let newText = text;
			newText = newText.trim().replace(/^ {2,}/gm, " ");
			newText = newText.replace(/(\r\n|\n|\r)/gm, "");
			newText = newText.replace('{ "', '{"');
			newText = newText.replace(': "', ':"');
			const serialized = serialize(newText.trim());
			parsed = serialized;
		} catch (error) {
			parsed = false;
	...
	jsonUnserialize(text: string) {
		...
		let parsed: string | boolean = "";
		try {
			const unserialized = unserialize(text.trim());
	...
	jsonDecode(text: string): string {
		try {
			const unserialized = unserialize(text);
