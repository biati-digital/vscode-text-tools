...
class LinesTools extends VSCodeTextTools {
    ...
    init(context: vscode.ExtensionContext): void {
        ...
        for (const command in commandsPrompt) {
            ...
            let disposable = vscode.commands.registerCommand(`biati.texttools.${command}`, async () => {
                const editor = vscode.window.activeTextEditor;
        ...
        // Add lines number
        let disposable = vscode.commands.registerCommand(`biati.texttools.addlinesnumber`, async () => {
            const editor = vscode.window.activeTextEditor;
     ...
     * Filter lines
     */
    filterLines(text: string, search: string, keep = true, caseSensitive = false): string {
        const parseSearch = regexSearchParser(search);
        const isRegex = parseSearch.isRegex;
        let isMatchStart = parseSearch.isMatchStart;
        let isMatchEnd = parseSearch.isMatchEnd;
        let isNot = parseSearch.isNot;
     ...
     */
    selectLinesMatching(text: string, search: string, selection: vscode.Selection): void {
        ...
        lines.forEach((line, index) => {
            ...
            if (match) {
                const lineAt = vscode.window?.activeTextEditor?.document.lineAt(new vscode.Position(index, 1));

                if (lineAt?.range) {
                    newRanges.push(new vscode.Selection(
...
function regexSearchParser(search: string, caseSensitive = false): {
