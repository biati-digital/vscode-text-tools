...
class GenerateTools extends VSCodeTextTools {
    ...
    init(context: vscode.ExtensionContext): void {
        ...
        disposable = vscode.commands.registerCommand(`biati.texttools.generatedummyfile`, async () => {
            ...
            const generate = await this.generateFile(workspaceDir, filename, filesize);
            if (generate) {
                this.showNotification({ message: `File generated correctly` });
            } else {
                this.showNotification({ message: `Error generating dummy file`, type: 'error' });
            }
        });
    ...
    async generateFile(filePath: vscode.Uri, fileName: string, size: string) {
        ...
        try {
            await this.execShellCommand('mkfile', ['-n', finalSize.toString(), finalFilePath]);
            if (isZip) {
